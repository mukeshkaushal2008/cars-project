...
class UploadHandler extends CI_Model{
    ...
    protected $error_messages = array(
        ...
        'min_width' => 'Image requires a minimum width 200px',
        'max_height' => 'Image exceeds maximum height',
        'min_height' => 'Image requires a minimum height 200px',
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
		$script_url=array();
		$script_url = 	explode('admin',$this->get_server_var('SCRIPT_FILENAME'));
		//echo $this->config->item("product_images_thumb");
		
        $this->options = array(
            'script_url' => $this->get_full_url() . '/',
            'upload_dir' => $script_url[0].'upload/',
			//'upload_url' => $this->get_full_url() . '/files/',
            'upload_url' => $this->config->item("upload"),//'../upload/message/',
			//'upload_dir' =>  $this->get_full_url().'/upload/',//ROOT_URL.'/upload/message/',
            'user_dirs' => false,
            'mkdir_mode' => 0777,
            'param_name' => 'files',
	...
	protected function new_name($file_path, $name, $size, $type, $error, $index, $content_range) {
    ...
    protected function handle_form_data($file, $index) {
	   ...
	   // $file->img;
		/*$file->message = mysql_real_escape_string($_REQUEST['message']);
		$file->message_displ = @$_REQUEST['message'];
		$file->message_sender = @$_REQUEST['message_sender']; //comes from hidden field
		$file->messag_reciver = @$_REQUEST['messag_reciver'];  //comes from hidden field
		$file->conversation = @$_REQUEST['conversation'];
		$file->img = user_image(@$file->message_sender);
		$file->username = get_name(@$file->message_sender);
		$file->tim = date('M d, g:i a',time()); 
		//echo debug($_REQUEST);
		//die;
		$file->uniq = random_generator(10);
		$file->messages_db = $file->message;*/
    }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
        ...
        //$file->name = $this->trim_file_name($uploaded_file, $name, $size, $type, $error, $index, $content_range);
	     $file->name = $this->new_name($uploaded_file, $name, $size, $type, $error, $index, $content_range);
		...
		/*$posted_type = explode('/',$file->type);
		//print_r($posted_type);
		$res = 'upload/message/'.$file->name;
		$text = ROOT_URL.'/images/docs.png';
		$application = ROOT_URL.'/images/docs.png';
		
		if($posted_type[0] == "image"){
			$f = $res;
		}
		else if($posted_type[0] == "text"){
			$f = $text;
		}
		else if($posted_type[0] == "application"){
			$f = $application;
		}
		$file->file_name=$f; */
